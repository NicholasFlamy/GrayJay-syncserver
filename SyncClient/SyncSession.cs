...
public class SyncSession : IDisposable, IAuthorizable
{
    ...
    {
        Logger.Info<SyncSession>($"Sent AUTHORIZED with session id {_id}");
    ...
    private void CheckAuthorized()
    {
        if (IsAuthorized)
        {
            ...
            Logger.Info<SyncSession>($"onAuthorized (isNewlyAuthorized = {isNewlyAuthorized}, isNewSession = {isNewSession})");
    ...
    public void HandlePacket(Opcode opcode, byte subOpcode, ReadOnlySpan<byte> data)
    {
            ...
            case Opcode.NOTIFY:
                switch ((NotifyOpcode)subOpcode)
                {
                    case NotifyOpcode.AUTHORIZED:
                        ...
                        Logger.Info<SyncSession>($"Received AUTHORIZED with session id {_remoteId} (device name: '{RemoteDeviceName ?? "not set"}')");
                        CheckAuthorized();
                        return;
        ...
        {
            Logger.Warning<SyncSession>($"Unknown opcode received: {opcode}");
            return;
        }
        ...
        catch (Exception e)
        {
            ...
            Logger.Warning<SyncSession>("Failed to handle packet", e);
        }
    }
    ...
    public async Task SendAsync(Opcode opcode, byte subOpcode, byte[]? data = null, int offset = 0, int count = -1, ContentEncoding contentEncoding = ContentEncoding.Raw, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var channel in channels) 
        {
            ...
            {
                Logger.Warning<SyncSession>($"Packet failed to send (opcode = {opcode}, subOpcode = {subOpcode}) due to no connected sockets, closing channel", e);
                channel.Dispose();
