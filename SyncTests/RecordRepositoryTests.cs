...
namespace SyncTests
{
    public abstract class RecordRepositoryTests
    {
        ...
        public async Task GetByPublishersAsync_ReturnsCorrectRecords()
        {
            ...
            var publisherKey3 = new byte[] { 10, 11, 12 };
            var key = "sharedKey";
            var blob1 = new byte[] { 13, 14, 15 };
            var blob2 = new byte[] { 16, 17, 18 };

            // Insert records for publisher1 and publisher2, but not for publisher3
            ...
            // Act
            var publishersToQuery = new[] { publisherKey1, publisherKey2, publisherKey3 };
            var records = await repo.GetByPublishersAsync(consumerKey, publishersToQuery, key);
            ...
            var record3 = recordList.FirstOrDefault(r => r.PublisherPublicKey.SequenceEqual(publisherKey3));

            Assert.IsNotNull(record1);
            Assert.IsNotNull(record2);
            Assert.IsNull(record3); // publisherKey3 should not have a record

            Assert.IsTrue(record1.EncryptedBlob.SequenceEqual(blob1));
            Assert.IsTrue(record2.EncryptedBlob.SequenceEqual(blob2));
        }
    }
